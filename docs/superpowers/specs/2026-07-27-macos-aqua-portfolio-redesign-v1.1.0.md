...
**Experience — `experience.tsx`.** Timeline rail kept; entries become `AquaWindow`s. Three
coloured badges per entry reduced to two graphite ones. **Edufied Pte added as the first
entry.** OEC Verify added, OWBAP corrected. The "Looking for Proven Delivery Experience?"
marketing block is replaced by a restrained Aqua window with résumé and contact links.
...
| B10 | **Emoji in a heading** — `Hi, I'm Josephus 👋` | 1 | Raised as a decision, not removed unilaterally — see §5.3. |

### 5.3 New material decisions arising from this audit

Three items in §5.1–5.2 change the design beyond v1.0.0 and need the owner's agreement:

1. **A1 — the badge/heading/subtitle stack is deleted on all six pages.** Page titles move into
   Aqua window title bars. This is the largest single visual change in the redesign, and it is
   also the one that most removes the generated look. It is coherent with the theme rather than
   a subtraction: in Mac OS X, the window title *is* the heading.
2. **A3 — `InteractiveCardWrapper` is retired.** The cursor-following glow is a scaffold
   signature and has no Mac OS X analogue. The file stays in the repo, unimported, so it is a
   one-line revert if the owner wants it back.
3. **B10 — the `👋` emoji in the hero H1.** A strong AI-portfolio tell, but it sits inside the
   headline whose animation the owner has tuned across five commits, and it may be a deliberate
   personal touch. Not removed without his say-so.
...
1. **C8 — location.** The résumé says *Cebu City, 6038*; the site says *Toledo City* and
   *Matab-ang, Toledo City*. The spec uses Cebu City to match the résumé. If Toledo City is
   correct, the résumé is the document that needs fixing.
2. **§5.3 — the three material decisions**: deleting the badge/heading/subtitle stack, retiring
   the spotlight wrapper, and whether the `👋` stays.

Noted, not acted on: `src/components/skills.tsx-revised` and a stray top-level `Users/`
directory look like leftovers.
